  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <UCard v-if="kycStatus" class="mb-6">
        <div class="flex items-center justify-between">
          ...
          <UIcon
            ...
            :class="[
              ...
              getStatusColor(kycStatus.status) === 'yellow' &&
                'text-yellow-500',
        ...
        <!-- Rejected Reason -->
        <div
          v-if="kycStatus.status === 'rejected' && kycStatus.review"
          class="mt-4 bg-red-50 dark:bg-red-900/20 rounded-lg p-4"
        >
        ...
        <!-- Approved Message -->
        <div
          v-if="kycStatus.status === 'approved'"
          class="mt-4 bg-green-50 dark:bg-green-900/20 rounded-lg p-4"
        >
        ...
        <!-- Pending Message -->
        <div
          v-if="kycStatus.status === 'pending'"
          class="mt-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4"
        >
      ...
      <UCard v-if="!kycStatus || kycStatus.status === 'rejected'">
        ...
        <form @submit.prevent="submitKyc" class="space-y-6">
          ...
          <!-- ID Card Number -->
          <UFormField label="เลขบัตรประชาชน" required>
            <UInput
          ...
          <!-- First Name -->
          <UFormField label="ชื่อจริง" required>
            <UInput
          ...
          <UFormField label="วันเกิด (ไม่บังคับ)">
            <UInput v-model="formData.idCardInfo.dateOfBirth" type="date" />
          </UFormField>
...
const fetchKycStatus = async () => {
  try {
    ...
    const response = await $fetch<KycStatus>(
      `${config.public.apiBase}/kyc/my-status`,
      {
        headers: {
